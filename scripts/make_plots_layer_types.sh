...
# 14, 'data/scenarios/analytical/layer_types/nonequal_mutov.json;analytic;KRFixedReqSize'
  ...
  SUFFIX=-n_base_cap_${SF}
  $PLOT $DATA $OUT_DIR -x4 -y1 -i0,1 --normal -p P_block ${GROUP} ${RANGE}  ${SAVE}  ${SUFFIX} ${SIZE3} ${Y_LIMITS} ${TC}
  SUFFIX=-n_base_size_${SF}
  $PLOT $DATA $OUT_DIR -x4 -y1 -i0,2 --normal -p P_block ${GROUP} ${RANGE}  ${SAVE}  ${SUFFIX} ${SIZE3} ${Y_LIMITS} ${TC}
  SUFFIX=-n_equal_cap_${SF}
  $PLOT $DATA $OUT_DIR -x4 -y1 -i3,4 --normal -p P_block ${GROUP} ${RANGE}  ${SAVE}  ${SUFFIX} ${SIZE3} ${Y_LIMITS} ${TC}
  SUFFIX=-n_equal_size_${SF}
  $PLOT $DATA $OUT_DIR -x4 -y1 -i3,5 --normal -p P_block ${GROUP} ${RANGE}  ${SAVE}  ${SUFFIX} ${SIZE3} ${Y_LIMITS} ${TC}
  SUFFIX=-n_equal_mut_cap_${SF}
  $PLOT $DATA $OUT_DIR -x4 -y1 -i6,7 --normal -p P_block ${GROUP} ${RANGE}  ${SAVE}  ${SUFFIX} ${SIZE3} ${Y_LIMITS} ${TC}
  SUFFIX=-n_equal_mut_size_${SF}
  $PLOT $DATA $OUT_DIR -x4 -y1 -i6,8 --normal -p P_block ${GROUP} ${RANGE}  ${SAVE}  ${SUFFIX} ${SIZE3} ${Y_LIMITS} ${TC}
  SUFFIX=-n_nonequal_cap_${SF}
  $PLOT $DATA $OUT_DIR -x4 -y1 -i9,10 --normal -p P_block ${GROUP} ${RANGE}  ${SAVE}  ${SUFFIX} ${SIZE3} ${Y_LIMITS} ${TC}
  SUFFIX=-n_nonequal_size_${SF}
  $PLOT $DATA $OUT_DIR -x4 -y1 -i9,11 --normal -p P_block ${GROUP} ${RANGE}  ${SAVE}  ${SUFFIX} ${SIZE3} ${Y_LIMITS} ${TC}
  SUFFIX=-n_nonequal_mut_cap_${SF}
  $PLOT $DATA $OUT_DIR -x4 -y1 -i12,13 --normal -p P_block ${GROUP} ${RANGE}  ${SAVE}  ${SUFFIX} ${SIZE3} ${Y_LIMITS} ${TC}
  SUFFIX=-n_nonequal_mut_size_${SF}
  $PLOT $DATA $OUT_DIR -x4 -y1 -i12,14 --normal -p P_block ${GROUP} ${RANGE}  ${SAVE}  ${SUFFIX} ${SIZE3} ${Y_LIMITS} ${TC}
